      ...
      <div className="max-w-[1120px] mx-auto px-6">
        <div className="grid grid-cols-[1fr_256px] gap-10 items-start pt-7 pb-16 max-[860px]:grid-cols-1">
          ...
          <div>
            ...
            <section className="panel-card p-5 mb-4">
              <div className="section-label mb-[10px]">{t.detail.description}</div>
              <p className="text-[14px] text-[var(--fg)] leading-[1.7]">{lang === "zh" && cmd.description_zh ? cmd.description_zh : cmd.description}</p>
            </section>

            {cmd.syntax && (
              <section className="panel-card p-5 mb-4">
                <div className="section-label mb-[10px]">{t.detail.syntax}</div>
            ...
            {cmd.parameters && cmd.parameters.length > 0 && (
              <section className="panel-card p-5 mb-4">
                <div className="section-label mb-[10px]">{t.detail.parameters}</div>
            ...
            {cmd.examples && cmd.examples.length > 0 && (
              <section className="panel-card p-5 mb-4">
                <div className="section-label mb-[10px]">{t.detail.examples}</div>
            ...
            {cmd.notes && cmd.notes.length > 0 && (() => {
              ...
              return (
                <section className="panel-card p-5 mb-4">
                  <div className="section-label mb-[10px]">{t.detail.notes}</div>
                  <ul className="flex flex-col gap-2">
                    {notes.map((n, i) => (
                      <li key={i} className="flex gap-[10px] p-[10px_12px] border border-[var(--border-light)] rounded-[var(--r)] text-[13px] leading-[1.6]">
                        <svg aria-hidden="true" className="flex-shrink-0 mt-[1px] text-[var(--accent)]" width="14" height="14" viewBox="0 0 15 15" fill="none">
            ...
            {cmd.caveats && cmd.caveats.length > 0 && (() => {
              ...
              return (
                <section className="panel-card p-5 mb-4">
                  <div className="section-label mb-[10px]">{t.detail.caveats}</div>
                  <ul className="flex flex-col gap-2">
                    {caveats.map((c, i) => (
                      <li key={i} className="flex gap-[10px] p-[10px_12px] border border-[var(--risk-med-bg)] rounded-[var(--r)] bg-[#fffcf0] dark:bg-[var(--risk-med-bg)] text-[13px] leading-[1.6]">
                        <svg aria-hidden="true" className="flex-shrink-0 mt-[1px] text-[var(--risk-med)]" width="14" height="14" viewBox="0 0 15 15" fill="none">
          ...
          <aside className="sticky top-[68px] max-[860px]:static">
            <div className="panel-card overflow-hidden mb-3">
            ...
            {cmd.source_url && (
              <div className="panel-card overflow-hidden mb-3">
                ...
                <div className="p-[12px_14px]">
                  <a href={cmd.source_url} target="_blank" rel="noopener noreferrer"
                    className="text-[12px] text-[var(--accent)] no-underline flex items-center gap-[5px] hover:underline">
                    <svg aria-hidden="true" width="12" height="12" viewBox="0 0 15 15" fill="none">
            ...
            {similar.length > 0 && (
              <div className="panel-card overflow-hidden mb-3">
                ...
                <div>
                  {similar.map((s, i) => (
                    <div key={s.id} className={`interactive-row flex items-center justify-between px-[14px] py-[9px] ${i < similar.length - 1 ? "border-b border-[var(--border-light)]" : ""}`}>
                      <Link href={`/commands/${s.tool_slug}/${s.slug}`} className="no-underline flex-1 min-w-0 focus-ring">
            ...
            <div className="mt-3 p-3 border border-dashed border-[var(--border)] rounded-[var(--r)] text-[12px] text-[var(--muted)] text-center">
              ...
              <a href="https://github.com/luoianun/ai-command-atlas/issues" target="_blank" rel="noopener noreferrer"
